...
class Helpers:
    def __init__(self, modelPath, dataDimension, numClasses = 2, optimizer=None, lossFuncs=None, metrics=None):
        ...
        for opt in self.optimizers:
            for loss in self.loss:
                for metric in self.metrics:
                    neuralOptimizerList.append(NeuralNet(self.modelPath, self.dataDimension, opt, loss, metric))
        return neuralOptimizerList
    
    def permuteMetrics(self, opt, loss):
        neuralOptimizerList = []
        for metric in itertools.permutations(self.metrics, 2):
            neuralOptimizerList.append(NeuralNet(self.modelPath, self.dataDimension, opt, loss, list(metric)))
        return neuralOptimizerList
...
class NeuralNet:
    ...
    """
    def __init__(self, modelPath, dataDim, opt=None, loss=None, metric=None):
        """
        Input:
    ...
    def createModel(self, dataDim, opt=None, loss=None, metric=None):
        ...
        model.add(tf.keras.layers.Dense(units=dataDim, activation=tf.nn.sigmoid))
        ...
        if metric == None:
            metric = ['categorical_accuracy', 'binary_crossentropy']
    ...
    def trainModel(self, Training_Data, Training_Labels, Testing_Data, Testing_Labels, epochs = 250, seeTrainingSteps = False):
    ...
    def scoreModel(self, signalData, signalLabels):
        diffData = self.predictData(signalData) - signalLabels
        numWrong = np.nansum(diffData/diffData)
        modelScore = (len(signalLabels) - numWrong)/len(signalLabels)

...
class supportVectorRegression:
    ...
    def createModel(self, modelType = "rbf"):
        if modelType == "linear":
            self.model = SVR(kernel="linear", C=100, gamma="auto")
        elif modelType == "rbf":
            self.model = SVR(kernel="rbf", C=100, gamma=0.1, epsilon=0.1)
        elif modelType == "poly":
            self.model = SVR(kernel="poly", C=100, gamma="auto", degree=self.polynomialDegree, epsilon=0.1, coef0=1)
        elif modelType == "sigmoid":
            self.model = SVR(kernel="sigmoid", C=100, gamma=0.1, epsilon=0.1)
        elif modelType == "precomputed":
            self.model = SVR(kernel="precomputed", C=100, gamma=0.1, epsilon=0.1)
        else:
